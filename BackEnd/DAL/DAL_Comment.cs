...
namespace DAL
{
    public class DAL_Comment : ITF_DAL_Comment
    {
        ...
        public List<Comment> get_comment_byid_post(int id)
        {
            ...
            try
            {
                var dt = _idbhelper.ExecuteSProcedureReturnDataTable(out msgError, "get_comment_byid_post", "@id", id);
                if (!string.IsNullOrEmpty(msgError))
                    throw new Exception(msgError);
        ...
        public List<Comment2> Search_Comment_User(int pageIndex, int pageSize, out long total, string Keywords)
        {
            ...
            try
            {
                var dt = _idbhelper.ExecuteSProcedureReturnDataTable(out msgError, "sp_Comment_search_User",
                    "@page_index", pageIndex,
                    "@page_size", pageSize,
                    "@Keywords", Keywords);
                if (!string.IsNullOrEmpty(msgError))
                    throw new Exception(msgError);
                if (dt.Rows.Count > 0) total = (long)dt.Rows[0]["RecordCount"];
                return dt.ConvertTo<Comment2>().ToList();
            }
            catch (Exception ex)
            {
                throw ex;
